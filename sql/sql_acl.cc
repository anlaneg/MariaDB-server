...
public:
  ...
  */
  ...
  ACL_ROLE(const char * rolename, Access privileges, MEM_ROOT *mem);
...
public:
  ...
  acl_host_and_ip host;
  const char *user,*db;
  Access initial_access; /* access bits present in the table */
...
static Access get_access(TABLE *form,uint fieldnr, uint *next_field=0);
...
class User_table: public Grant_table_base
{
 public:
  ...
  virtual Access get_access() const = 0;
  virtual void set_access(const Access rights, bool revoke) const = 0;
...
class User_table_tabular: public User_table
{
 public:
  ...
  Access get_access() const
  {
    Access access(Grant_table_base::get_access());
  ...
  void set_access(const Access rights, bool revoke) const
  {
...
class User_table_json: public User_table
{
 ...
 private:
  ...
  Access get_access_value(const char *key) const
  {
    return Access(ALL_KNOWN_ACL & (ulonglong) get_int_value(key));
  }
...
ACL_ROLE::ACL_ROLE(const char * rolename, Access privileges, MEM_ROOT *root) :
...
static Access get_access(TABLE *form, uint fieldnr, uint *next_field)
{
...
static int check_user_can_set_role(const char *user, const char *host,
                      const char *ip, const char *rolename, Access *access)
{
  ACL_ROLE *role;
...
int acl_check_setrole(THD *thd, const char *rolename, Access *access)
{
...
int acl_setrole(THD *thd, const char *rolename, Access access)
{
...
static void acl_update_role(const char *rolename, const Access privileges)
{
...
static void acl_insert_role(const char *rolename, Access privileges)
{
  ACL_ROLE *entry;
...
Access acl_get(const char *host, const char *ip,
               const char *user, const char *db, my_bool db_is_pattern)
{
  Access host_access(ALL_KNOWN_ACL), db_access(NO_ACL);
  uint i;
  size_t key_length;
...
static int replace_user_table(THD *thd, const User_table &user_table,
                              LEX_USER * const combo, Access rights,
...
static int replace_db_table(TABLE *table, const char *db,
			    const LEX_USER &combo,
			    Access rights, const bool revoke_grant)
{
  uint i;
  ulonglong priv;
  Access store_rights(NO_ACL);
  bool old_row_exists=0;
  int error;
...
public:
  ...
  Access init_rights;
  uint key_length;
  GRANT_COLUMN(String &c, Access y) :rights (y), init_rights(y)
  {
...
public:
  ...
  Access privs;
  Access init_privs; /* privileges found in physical table */
  ulonglong sort;
  size_t key_length;
  GRANT_NAME(const char *h, const char *d,const char *u,
             const char *t, Access p, bool is_routine);
...
static Access get_access_value_from_val_int(Field *field)
{
  return Access(ALL_KNOWN_ACL & (ulonglong) field->val_int());
}
...
public:
  Access cols;
  Access init_cols; /* privileges found in physical table */
  HASH hash_columns;

  GRANT_TABLE(const char *h, const char *d,const char *u,
              const char *t, Access p, Access c);
  ...
  */
  ...
  if (cols)
  {
    ...
    do
    {
      ...
      Access priv= get_access_value_from_val_int(col_privs->field[6]);
...
static int replace_column_table(GRANT_TABLE *g_t,
				...
				Access rights, bool revoke_grant)
{
  int result=0;
  uchar key[MAX_KEY_LENGTH];
  ...
  while ((column= iter++))
  {
    Access privileges= column->rights;
    bool old_row_exists=0;
    uchar user_key[MAX_KEY_LENGTH];
    ...
    {
      Access tmp= get_access_value_from_val_int(table->field[6]);
  ...
  */
  ...
  if (revoke_grant)
  {
    ...
    do
    {
      Access privileges = get_access_value_from_val_int(table->field[6]);
...
static int replace_table_table(THD *thd, GRANT_TABLE *grant_table,
			       TABLE *table, const LEX_USER &combo,
			       const char *db, const char *table_name,
			       Access rights, Access col_rights,
			       bool revoke_grant)
{
  char grantor[USER_HOST_BUFF_SIZE];
  int old_row_exists = 1;
  int error=0;
  Access store_table_rights(NO_ACL), store_col_rights(NO_ACL);
  uchar user_key[MAX_KEY_LENGTH];
  ...
  */
  ...
  {
    store_record(table,record[1]);
    Access j= get_access_value_from_val_int(table->field[6]);
    Access k= get_access_value_from_val_int(table->field[7]);
			      ...
			      Access rights, bool revoke_grant)
{
  ...
  */
  ...
  Access store_proc_rights= get_rights_for_procedure(rights);
  if (old_row_exists)
  {
    store_record(table,record[1]);
    Access j= get_access_value_from_val_int(table->field[6]);
...
*/
static int update_role_db(int merged, int first, Access access,
                          const char *role)
{
  if (first < 0)
    return 0;
...
{
  Access rights __attribute__((unused)) (NO_ACL);
  int changed= 0;
  if (!merged->cols)
  {
                        ...
                        const char *db, const char *tname, role_hash_t *rhash)
{
  ...
  */
  ...
  Access privs(NO_ACL), cols(NO_ACL);
  ulong update_flags= 0;
...
static int update_role_routines(GRANT_NAME *merged, GRANT_NAME **first,
                                Access privs, const char *role, HASH *hash)
{
  if (!first)
    return 0;
...
int mysql_table_grant(THD *thd, TABLE_LIST *table_list,
		      ...
		      List <LEX_COLUMN> &columns, Access rights,
		      bool revoke_grant)
{
  Access column_priv(NO_ACL);
  int result;
...
bool mysql_routine_grant(THD *thd, TABLE_LIST *table_list,
                         const Sp_handler *sph,
			 List <LEX_USER> &user_list, Access rights,
...
bool mysql_grant(THD *thd, const char *db, List <LEX_USER> &list,
                 Access rights, bool revoke_grant, bool is_proxy)
{
  ...
  while ((tmp_Str = str_list++))
  {
    ...
    {
      Access db_rights(rights & DB_ACLS);
      if (db_rights  == rights)
      {
...
bool check_grant(THD *thd, Access want_access, TABLE_LIST *tables,
                 bool any_combination_will_do, uint number, bool no_errors)
{
  ...
  Security_context *sctx= thd->security_ctx;
  uint i;
  Access original_want_access(want_access);
  bool locked= 0;
  GRANT_TABLE *grant_table;
  ...
  */
  ...
  for (tl= tables; number-- ; tl= tl->next_global)
  {
    ...
    Access orig_want_access(original_want_access);
			...
			const char *name, size_t length,  Security_context *sctx)
{
  ...
  GRANT_COLUMN *grant_column;
  Access want_access(grant->want_privilege & ~grant->privilege);
...
*/
bool check_grant_all_columns(THD *thd, Access want_access_arg,
                             Field_iterator_table_ref *fields)
{
  Security_context *sctx= thd->security_ctx;
  Access want_access(NO_ACL);
...
bool check_grant_routine(THD *thd, Access want_access,
...
Access get_table_grant(THD *thd, TABLE_LIST *table)
{
  Security_context *sctx= thd->security_ctx;
  ...
  Access privilege(table->grant.privilege);
  mysql_rwlock_unlock(&LOCK_grant);
  return privilege;
}
...
Access get_column_grant(THD *thd, GRANT_INFO *grant,
                        const char *db_name, const char *table_name,
                                   ...
                                   char *buff, size_t buffsize)
{
  ...
  else
  {
    bool found=0;
    ulonglong j;
    Access test_access(want_access & ~GRANT_ACL);
  ...
  for (index=0 ; index < column_priv_hash.records ; index++)
  {
    ...
    */
    ...
    if (!strcmp(username,user) &&
        !my_strcasecmp(system_charset_info, hostname, host))
    {
      Access table_access(NO_ACL);
      Access cols_access(NO_ACL);
      if (*hostname) // User
      {
      ...
      {
        String global(buff, sizeof(buff), system_charset_info);
        Access test_access= (table_access | cols_access) & ~GRANT_ACL;
                               ...
                               char *buff, int buffsize)
{
  ...
  for (index=0 ; index < hash->records ; index++)
  {
    ...
    */
	...
	Access test_access(proc_access & ~GRANT_ACL);
...
void get_privilege_desc(char *to, uint max_length, Access access_arg)
{
  uint pos;
  char *start=to;
...
{ return 0; }
bool check_grant(THD *, Access, TABLE_LIST *, bool, uint, bool)
{ return 0; }
...
bool Sql_cmd_grant_object::grant_stage0_exact_object(THD *thd,
                                                     TABLE_LIST *table)
{
  Access priv= m_object_privilege | m_column_privilege_total | GRANT_ACL;
...
bool Sql_cmd_grant_sp::execute(THD *thd)
{
  ...
  Access grants= m_all_privileges
...
int fill_schema_user_privileges(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (counter=0 ; counter < acl_users.elements ; counter++)
  {
    ...
    Access want_access(acl_user->access);
    if (!(want_access & GRANT_ACL))
      is_grantable= "NO";
...
int fill_schema_schema_privileges(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (counter=0 ; counter < acl_dbs.elements() ; counter++)
  {
    ...
    Access want_access(acl_db->access);
    if (want_access)
    {
      if (!(want_access & GRANT_ACL))
      {
...
int fill_schema_table_privileges(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (index=0 ; index < column_priv_hash.records ; index++)
  {
    ...
    Access table_access(grant_table->privs);
    if (table_access)
    {
      Access test_access(table_access & ~GRANT_ACL);
      /*
...
int fill_schema_column_privileges(THD *thd, TABLE_LIST *tables, COND *cond)
{
  ...
  for (index=0 ; index < column_priv_hash.records ; index++)
  {
    ...
    Access table_access(grant_table->cols);
